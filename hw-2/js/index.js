...
if (isNaN(sideOne) || sideOne <= 0 || isNaN(sideTwo) || sideTwo <= 0 || isNaN(sideThree) || sideThree <=0) {
...
if ((Math.pow(sideOne, 2) + Math.pow(sideTwo, 2)) == (Math.pow(sideThree, 2)) || (Math.pow(sideThree, 2) + Math.pow(sideOne, 2)) == (Math.pow(sideTwo, 2)) || (Math.pow(sideThree, 2) + Math.pow(sideTwo, 2)) == (Math.pow(sideOne, 2))) {
  alert("Y");
} else {
  alert("N");
}
...
function checkTime(time) {
	...
	let message = (time > 17 && time <= 23) ? "evening!" :
		(time > 5 && time <= 11) ? "morning!" :
		(time > 11 && time <= 17) ? "day!" :
		"night!";
	alert(message);
